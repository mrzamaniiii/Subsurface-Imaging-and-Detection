...
Nr = size(recfield.data, 2); % Number of receivers

energy = zeros(1, Nr);
for i = 1:Nr
    trace = recfield.data(:, i); % Total time for receiver i
    energy(i) = sum(trace.^2) * dt;   % energy of trace
end

% Suggestion

energy_n = energy / energy(3); % Normalization of energies relative to the first recciever

% Seismic traces

figure;
seisplot2(recfield.data, recfield.time, offsets, 1, 0, 1);
xlabel('Offset (m)');
ylabel('Time (s)');
...
grid on;
hold on
plot(offsets,1./offsets * offsets(3))