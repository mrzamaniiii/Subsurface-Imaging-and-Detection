...
lens_mask = ((X - lens_center_x).^2)/(lens_a^2) + ...
            ((Z - lens_center_z).^2)/(lens_b^2) <= 1; % Ellipse equation

v_lens = 2000; % Velocity in Lens
...
t0_raw = -(t0_raw) + max(t0_raw);
source.t0 = t0_raw ; % beamforming delay
...
mid = 2;
[~, idx0] = max(abs(tr(:,mid))) % It is the moment when the waves are most concentrated in the target. 0.721 s. The window for the focus is 0.691 to 0.751
...
Gain_win   = Ewin_at / ((Ewin_before + Ewin_after)/2); % The energy on the target is 26 times greater than the surrounding area.
FocusRatio = Ewin_at / (Ewin_before + Ewin_at + Ewin_after); % 93 percent of the three-point energy is focused on the target.

% SANITY CHECKS
FullE = sum(tr.^2) * dt; % we focus energy in time rahther than maximizing peak amplitude. The total energy on the target is only 12% of the energy before the target.
PeakA = max(abs(tr));
